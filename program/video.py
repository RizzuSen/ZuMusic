...
async def vplay(c: Client, m: Message):
    ...
    if m.sender_chat:
        return await m.reply_text("Anda adalah Admin Anonim!\n\n»Silahkan Kembali ke akun pengguna dari hak admin(bukan Admin Anonim).")
    try:
        aing = await c.get_me()
    except Exception as e:
    ...
    if a.status != "administrator":
        await m.reply_text(
            f" 💡 Untuk menggunakan saya, saya harus menjadi Administrator dengan izin berikut:\n\n» ❌ Hapus pesan\n» Batasi pengguna\n» ❌ Tambahkan pengguna\n» Kelola obrolan video\n\nData akan diperbarui secara otomatis setelah Anda **Menambahkan Saya Sebagai Admin**"
        )
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await m.reply_text(
                f"@{ASSISTANT_NAME} **telah dibanned dari group** {m.chat.title}\n\n» **unban userbot Terlebih dahulu Untuk menggunakan bot ini.**"
            )
            return
...
async def vstream(c: Client, m: Message):
    ...
    if a.status != "administrator":
        await m.reply_text(
            f"💡Untuk menggunakan saya, saya harus menjadi Administrator dengan izin berikut:\n\n» ❌ Hapus pesan\n» Batasi pengguna\n» ❌ Tambahkan pengguna\n» Kelola obrolan video\n\nData adalah diperbarui secara otomatis setelah Anda **menambahkan saya sebagai admin**"
        )
        return
    if not a.can_manage_voice_chats:
        await m.reply_text(
